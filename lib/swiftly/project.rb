      ...
      def self.settings( project_name )
        ...
        settings = {
          ...
          local: {
            ...
            db_host: defined?(wp_config[:local][:db_host]) ? wp_config[:local][:db_host] : global_config[:local][:db_host],
            db_user: defined?(wp_config[:local][:db_user]) ? wp_config[:local][:db_user] : global_config[:local][:db_user],
            db_pass: defined?(wp_config[:local][:db_pass]) ? wp_config[:local][:db_pass] : global_config[:local][:db_pass],
          },
          staging: {
            db_name:  defined?(wp_config[:staging][:db_name]) ? wp_config[:staging][:db_name] : project_config[:staging][:db_name],
            db_host:  defined?(wp_config[:staging][:db_host]) ? wp_config[:staging][:db_host] : global_config[:staging][:db_host],
            db_user:  defined?(wp_config[:staging][:db_user]) ? wp_config[:staging][:db_user] : global_config[:staging][:db_user],
            db_pass:  defined?(wp_config[:staging][:db_pass]) ? wp_config[:staging][:db_pass] : global_config[:staging][:db_pass],
            domain:   defined?(wp_config[:staging][:domain])  ? wp_config[:staging][:domain]  : global_config[:staging][:domain],
          ...
          production: {
            ...
            db_host:  defined?(wp_config[:production][:db_host]) ? wp_config[:production][:db_host] : global_config[:production][:db_host],
            db_user:  defined?(wp_config[:production][:db_user]) ? wp_config[:production][:db_user] : global_config[:production][:db_user],
            db_pass:  defined?(wp_config[:production][:db_pass]) ? wp_config[:production][:db_pass] : global_config[:production][:db_pass],
            domain:   defined?(wp_config[:production][:domain])  ? wp_config[:production][:domain]  : global_config[:production][:domain],
