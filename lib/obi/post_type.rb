...
module Obi
	class PostType < Thor::Group
		...
		def create_post
			...
			template File.join( 'templates', 'post_type.erb' ), File.join( "#{project_path}", 'wp-content', 'plugins', "#{project_name}-specific-plugin", 'custom-post-types',"#{post_type.pluralize}.php")
		end
	end
end